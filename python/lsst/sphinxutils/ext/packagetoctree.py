...
class ModuleTocTree(Directive):
    ...
    **Options**
    ...
    def run(self) -> list[nodes.Node]:
        ...
        logger.debug("module-toctree found %d modules", len(module_index_files))
        ...
        wrappernode = nodes.compound(classes=["toctree-wrapper", "module-toctree"])
    ...
    def _parse_skip_option(self) -> list[str]:
    ...
    def _parse_module_name(self, docname: str) -> str:
...
class PackageTocTree(Directive):
    ...
    **Options**
    ...
    def run(self) -> list[nodes.Node]:
        ...
        logger.debug("package-toctree found %d packages", len(package_index_files))
        ...
        wrappernode = nodes.compound(classes=["toctree-wrapper", "package-toctree"])
        wrappernode.append(subnode)
        self.add_name(wrappernode)
        new_nodes.append(wrappernode)
    ...
    def _parse_package_name(self, docname: str) -> str:
...
def setup(app: Sphinx) -> ExtensionMetadata:
    """Set up ``module-toctree`` and ``package-toctree`` directives."""
